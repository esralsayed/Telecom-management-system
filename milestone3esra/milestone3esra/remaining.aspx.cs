        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
                try {
                    ...
                    SqlCommand remaining = new SqlCommand(query, conn);
                    remaining.Parameters.AddWithValue("@mobile_num", mobileNo);
                    remaining.Parameters.AddWithValue("@plan_name", planName);
                    object output = remaining.ExecuteScalar();
                    if (output != null && int.TryParse(output.ToString(), out int remainingAmount))
                    {
                        responseR.Text = $"Your remiaining amount is:{remainingAmount}";
                    }
                    else
                    {
                        responseR.Text = "Remaining Amount = 0";
                    }
